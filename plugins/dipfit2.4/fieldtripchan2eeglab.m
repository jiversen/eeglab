...
function chanlocs = fieldtripchan2eeglab( loc );
    ...
    try % *** JRI *** original EEGLAB code now fails because fields have been renamed
        chanlocs = struct('labels', loc.label(:)', ...
            'X', mattocell(loc.pnt(:,1)'), ...
            'Y', mattocell(loc.pnt(:,2)'), ...
            'Z', mattocell(loc.pnt(:,3)'));
    catch %new format of fieldtrip channel structures
        chanlocs = struct('labels', loc.label(:)', ...
            'X', mattocell(loc.chanpos(:,1)'), ...
            'Y', mattocell(loc.chanpos(:,2)'), ...
            'Z', mattocell(loc.chanpos(:,3)'));
        if isfield(loc,'type'),
            [chanlocs.type] = deal(loc.type);
        end
    end
